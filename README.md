...
- attaches the loader script (`<endpoint>/widget.js`) only on admin routes
  (since 1.2 — frontend pages never load it, so it can't collide with a
  site's own visitor chat widget) and only for roles with the
  **Use support assistant** permission (anonymous users never load it);
...
Defaults to the production app (`https://support.originoutside.com`).
Point local dev at a local app build
